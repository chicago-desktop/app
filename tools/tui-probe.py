#!/usr/bin/env python3
"""A probe for a TUI: runs a command in a real PTY and takes the screen as text.

The same idea as a browser probe: a full-screen program cannot be checked by
reading its code or its exit code. `wippy run ... windows` does not start at
all without a real terminal, and once started it writes not lines but an ANSI
stream with absolute positioning.

The probe provides a PTY of the given size, types by a script, and parses the
stream into a grid of characters. It parses exactly what the runtime's surface
writes:
...
class Ordered(argparse.Action):
    """Collects the steps into ONE list in the order they were given.

    The script used to be assembled by type — first every --send, then the
    resizes, then the keys — and "click, then type" had no way to be said:
    it came out as "type, then click", silently and wrong.
    """
...
class Screen:
    """A minimal screen: positioning, printing, erasing to the end of the line."""
...
def main():
    parser = argparse.ArgumentParser(
        description="Run a command in a real PTY, type by a script, print the screen as text.")
    ...
    parser.add_argument("--settle", type=float, default=2.0,
                        help="pause between the steps of the script, seconds")
    parser.add_argument("--tail", type=float, default=None,
                        help="how long to wait after the last step, seconds "
                             "(default: one pause; the exit of the whole runtime "
    ...
    parser.add_argument("--click", action=Ordered, default=[],
                        help="click the mouse at COL,ROW (one-based coordinates)")
    ...
    parser.add_argument("--move", action=Ordered, default=[],
                        help="move the mouse to COL,ROW without a button (SGR 1003, button 35)")
    ...
    parser.add_argument("--resize", action=Ordered, default=[],
                        help="change the terminal size to COLSxROWS (repeatable)")
    ...
    # The script: [(moment, what to send)] in the order the steps were given
    # on the command line. The first step comes after boot.
    def point(spec, what):
    ...
    for kind, value in getattr(opts, "steps", []):
        if kind == "send":
            ...
            col, row = point(value, "click")
            script.append((moment, mouse(col, row, press=True) + mouse(col, row, press=False)))
        elif kind == "dblclick":
            col, row = point(value, "double-click")
            # Two full clicks in one chunk: a double-click is recognized by the
            # time between them, and the script's pause between steps would
            # break it.
            single = mouse(col, row, press=True) + mouse(col, row, press=False)
            script.append((moment, single + single))
        elif kind == "move":
            col, row = point(value, "move")
            ...
            # One chunk: press, motion with the button held (SGR: button + 32)
            ...
            if not match:
                parser.error("the size is given as COLSxROWS, got: " + value)
    ...
    try:
        while time.time() < deadline:
            if step < len(script) and time.time() - started >= script[step][0]:
                ...
                if isinstance(action, tuple):
                    ...
                    # process group, and the screen is rebuilt for the new size.
                    _, cols, rows = action
        ...
        # Whether it exited by itself is the only way to tell a proper exit on
    ...
    if exited_on_its_own:
        print(f"[probe] the program exited by itself, code {exit_code}")
    else:
        print("[probe] the program did not exit — SIGTERM was sent")

    rendered = screen.render()
    failed = [needle for needle in opts.expect if needle not in rendered]
    for needle in opts.expect:
        print(f"[probe] {'MISSING' if needle in failed else 'present'}: {needle!r}")

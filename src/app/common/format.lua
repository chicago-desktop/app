-- Time, names and refusal text for the stand's windows (app.common:format).
--
...
-- Pure: no IO, the window passes the clock's offset in
...
-- the logon (app.desktop:logon, the name the Start menu starts with) and for
-- the refresh after a rename (app.desktop:user_name): two copies of it would
-- make the Start menu change a name that nobody changed.

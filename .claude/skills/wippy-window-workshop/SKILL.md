...
(`POST /api/public/user/token` with an account's e-mail and password, see
`.env.example`): `POST /api/v1/tui-desktop/apps` takes the same fields as
`build`; `GET /api/v1/tui-desktop/apps`, `DELETE /api/v1/tui-desktop/apps/{name}`.
Open and inspect through the desktop command channel — skill `tui-desktop`.
...
- **Pixels:** `screen` is empty; the evidence is a PNG rendered by the same
  ...
  `run_shot` in the shell's `test/src/paint_png.lua` does. Give every shot a new

...
Use the [SDK documentation](../../../docs/sdk.md) — the application's copy of
the shell's `docs/sdk.md` (canonical in `windows/shell`,
[wippy-windows/windows](https://github.com/wippy-windows/windows); the copy
says which version it was taken from). Resolve this skill's symlink before
following relative paths. The documentation and this skill are versioned
together in `windows/shell`; do not copy their contracts into per-app notes.
See [the existing-window audit](https://github.com/wippy-windows/windows/blob/main/docs/sdk-audit-2026-09-08.md)
...
3. Declare one `process.lua` with `meta.type: tui_desktop.window`, exact title,
   ...
   pack (`<pack entry>/<file>`, the shell's docs/icons.md "Image packs"); a module's

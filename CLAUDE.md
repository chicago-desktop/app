...
- **It runs only on a build of the runtime fork** — wippy-windows/runtime,
...
- **`--host` on every command:** `wippy run --host windows.shell:terminal windows`
...
- **Entry ids named by environment variables stay put:** `app.desktop:logon`,
  ...
  (see the `WINDOWS_*` overrides in `.wippy.yaml`).
